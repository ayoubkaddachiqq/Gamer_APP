...
public class AjouterAnnonceController {
    ...
    private String corrigerTexte(String texte) {
        ...
        Matcher matcher = pattern.matcher(texteCorrige);
        StringBuffer resultat = new StringBuffer();

        while (matcher.find()) {
            String mot = matcher.group();
            String correction = trouverCorrection(mot);
            matcher.appendReplacement(resultat, Matcher.quoteReplacement(correction));
        }
    ...
    private String corrigerPhrases(String texte) {
        ...
        for (Map.Entry<String, String> entry : correctionsPhrases.entrySet()) {
            Pattern pattern = Pattern.compile("(?iu)\\b" + Pattern.quote(entry.getKey()) + "\\b");
            Matcher matcher = pattern.matcher(resultat);
            StringBuffer buffer = new StringBuffer();

            while (matcher.find()) {
                matcher.appendReplacement(buffer, Matcher.quoteReplacement(appliquerCassePhrase(matcher.group(), entry.getValue())));
            }
    ...
    private String trouverCorrection(String mot) {
        ...
        if (correction == null) {
            ...
            if (meilleureDistance > distanceMax) {
                return mot;
            }
        }
